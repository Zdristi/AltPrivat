...
async def referral_command(message: Message):
    referral_link = f"https://t.me/{message.from_user.username or 'username_not_set'}?start={message.from_user.id}"
...
@dp.callback_query(lambda c: c.data == 'show_tariff_main')
async def show_tariff_handler(callback_query: types.CallbackQuery):
...
@dp.callback_query(lambda c: c.data == 'show_tariff_photo_legs')
async def show_tariff_photo_legs_handler(callback_query: types.CallbackQuery):
    await callback_query.answer()
...
@dp.callback_query(lambda c: c.data == 'show_tariff_juicy_altushki')
async def show_tariff_juicy_altushki_handler(callback_query: types.CallbackQuery):
    await callback_query.answer()
...
@dp.callback_query(lambda c: c.data == 'show_tariff_mini_sampler')
async def show_tariff_mini_sampler_handler(callback_query: types.CallbackQuery):
    await callback_query.answer()
...
async def back_to_tariffs_handler(callback_query: types.CallbackQuery):
    await callback_query.answer()
...
async def payment_card_handler(callback_query: types.CallbackQuery):
    await callback_query.answer()
...
@dp.callback_query(lambda c: c.data == 'payment_stars')
async def payment_stars_handler(callback_query: types.CallbackQuery):
    await callback_query.answer()
    ...
    try:
        await bot.send_invoice(
            ...
            prices=[types.LabeledPrice(label=f'Подписка на {SUBSCRIPTION_DAYS} дней', amount=int(SUBSCRIPTION_PRICE))]
        )
    except Exception as e:
...
@dp.callback_query(lambda c: c.data == 'payment_crypto')
async def payment_crypto_handler(callback_query: types.CallbackQuery):
    await callback_query.answer()
...
@dp.callback_query(lambda c: c.data == 'subscribe_channel')
async def process_subscribe_channel(callback_query: types.CallbackQuery):
    await callback_query.answer()
...
# ПРИМЕЧАНИЕ: Для автоматизации оплаты украинской картой:
# 1. PayGuru (https://payguru.ua):
#    - Интеграция с банками Украины (Приват24, Монобанк)
#    - Автоматическая проверка платежей
#    - Пример интеграции: https://payguru.ua/api
# 2. LiqPay (https://www.liqpay.ua):
#    - Решение от ПриватБанка
#    - Поддержка UAH (гривна)
#    - Автоматическая обработка платежей
# 3. Stripe с поддержкой украинских карт:
#    - Требует верификацию, но поддерживает UAH
# 4. CryptoBot с оплатой в UAH:
#    - Можно принимать гривны через CryptoBot
#
# Для интеграции создайте новый обработчик оплаты (payment_card_handler)
# с вызовом соответствующего API и проверкой успешной оплаты.